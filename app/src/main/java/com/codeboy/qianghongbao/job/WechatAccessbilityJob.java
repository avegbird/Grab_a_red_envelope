...
public class WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    private static final String QQ_HONGBAO_TEXT_KEY = "[微信红包]";
    ...
    public void onReceiveJob(AccessibilityEvent event) {
        ...
        if(eventType == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED) {
            ...
            Log.e(Config.TAG,"Parcelable = "+data);
            ...
            if(!texts.isEmpty()) {
                ...
                Log.e(Config.TAG,"通知栏text="+text);
                notificationEvent(text, (Notification) data);
            }
        } else if(eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {//界面打开
            openHongBao(event);
        } else if(eventType == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED) {//界面变化
    ...
    private void notificationEvent(String ticker, Notification nf) {
        ...
        text = text.trim();
        Log.e(Config.TAG,"通知栏截取的信息="+text);
    ...
    private void openHongBao(AccessibilityEvent event) {
        Log.e(Config.TAG,"getClassName()="+event.getClassName());
        if("com.tencent.mm.plugin.luckymoney.ui.En_fba4b94f".equals(event.getClassName())) {
            ...
            if(getConfig().getWechatAfterGetHongBaoEvent() == Config.WX_AFTER_GET_GOHOME) { //返回主界面，以便收到下一次的红包通知
                AccessibilityHelper.performBack(getService());//先退出红包，防止下次进入微信出bug
    ...
    private void handleLuckyMoneyReceive() {
        ...
        if(event == Config.WX_AFTER_OPEN_HONGBAO) { //拆红包
            if (wechatVersion < USE_ID_MIN_VERSION) {
                ...
                }else if(wechatVersion == 1080){
                    buttonId = "com.tencent.mm:id/bp6";
                }
